...
class DataValidation:
    ...
    def validate_no_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Required no of columns: {number_of_cols}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            ## validate if numerical cols exist or not
            numeric_cols = self._schema_config['numerical_columns']
            missing_train_cols = [col for col in numeric_cols if col not in train_dataframe.columns]
            missing_test_cols = [col for col in numeric_cols if col not in test_dataframe.columns]

            if len(missing_train_cols) == 0  and len(missing_test_cols) == 0:
                error_message= f"Train and Test Dataframe doesnot contain all numeric columns"        
